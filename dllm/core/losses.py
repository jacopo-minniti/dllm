...
class LoopholingBPTTLoss(nn.Module):
    ...
    def forward(self, model, batch, **kwargs):
        ...
        for t in range(self.num_steps):
            ...
            # Wrap all-but-last BPTT forward in no_sync() so DDP's _rebuild_buckets()
            # doesn't fire on intermediate steps before the single backward() call.
            _sync_ctx = (
                model.no_sync()
        ...
        for t in range(self.num_steps):
            ...
            if self.weighted_ce:
                with torch.no_grad():
                    ...
                    if self.confidence_type == "top_prob":
                        ...
                        raise ValueError(f"Unsupported confidence_type={self.confidence_type!r}")
                    weights = 1.0 + conf
            ...
            if t < self.num_steps - 1:
                with torch.no_grad():
                    ...
                    # Out-of-place update avoids autograd version mismatch (Embedding saves indices)
