...
class ZeroBridgeRMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not getattr(self, '_params_verified', False):
            if (~torch.isfinite(self.gamma)).any() or (~torch.isfinite(self.beta)).any():
                warnings.warn(
                    "ZeroBridgeRMSNorm: non-finite parameters detected "
...
class CrossAttentionBridge(nn.Module):
    def __init__(self, config):
        ...
        assert d_b % self.n_heads == 0, f"cab_bottleneck_dim must be divisible by cab_n_heads"
...
class LoopholeModule(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        rms_eps = getattr(config, "rms_norm_eps", 1e-6)
        self.zero_bridge = ZeroBridgeRMSNorm(size=config.hidden_size, eps=rms_eps)
        self.mlp = None
