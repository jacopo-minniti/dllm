...
@dataclass
class Fast_dLLMEvalSamplerConfig(MDLMSamplerConfig):
    """Default sampler config for Fast_dLLM eval."""
...
class Fast_dLLMEvalConfig(MDLMEvalConfig):
    """Fast_dLLM eval config."""

    max_length: int = 4096

@register_model("fastdllm_v2")
class Fast_dLLMEvalHarness(MDLMEvalHarness):
    def __init__(
        self,
        eval_config: Fast_dLLMEvalConfig | None = None,
        sampler_config: MDLMSamplerConfig | None = None,
        sampler_cls: type[MDLMSampler] = MDLMSampler,
        **kwargs,
    ):
        eval_config = eval_config or Fast_dLLMEvalConfig()
        sampler_config = sampler_config or Fast_dLLMEvalSamplerConfig()
